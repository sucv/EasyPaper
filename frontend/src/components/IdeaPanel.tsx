...
import type { IdeaState, IdeaPaper, BusyState, WsEvent, TaskConfig, ModelConfig } from '../types';
...
export default function IdeaPanel({ projectId, idea, busy, onDelete, addListener, onRefresh }: Props) {
  ...
  const fetchConfig = async () => {
    try {
      ...
      if (tasksData.tasks?.length && !selectedTask) setSelectedTask(tasksData.tasks[0].task_id);
      if (modelsData.models?.length && !selectedModel) setSelectedModel(modelsData.models[0].id);
    } catch {}
  };
  ...
  const handleDeleteReport = async (filename: string, title: string) => {
    if (!confirm(`Delete report "${title}"?\n\nThis will permanently remove the report file. This action cannot be undone.`)) return;
    try {
      await fetch(`${API}/projects/${projectId}/ideas/${idea.idea_slug}/reports/${filename}`, { method: 'DELETE' });
      if (viewFile === filename) handleCloseViewer();
      fetchFiles();
    } catch {}
  };
    ...
    <div className="space-y-5">
      ...
      {showGuide && (
        <div className="bg-indigo-50/60 border border-indigo-100 rounded-lg px-4 py-3 flex items-start gap-3">
          ...
          <div className="flex-1 text-xs text-indigo-800 leading-relaxed">
            ...
            <span className="mx-1">③</span><strong>Research</strong> — select a task and model to generate a report.
          </div>
      ...
      <div>
        ...
        {papers.length > 0 ? (
                ...
                {papers.map((p, i) => {
                  ...
                  return (
                    <tr key={p.paper_id} className={`h-10 transition-colors ${isViewingThis ? 'bg-blue-50 border-l-2 border-l-blue-400' : i % 2 === 1 ? 'bg-gray-50/40' : ''} hover:bg-indigo-50/20`}>
                      <td className="px-2 py-1.5"><input type="checkbox" className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" checked={poolSelected.has(p.paper_id)} onChange={() => setPoolSelected(prev => { const next = new Set(prev); next.has(p.paper_id) ? next.delete(p.paper_id) : next.add(p.paper_id); return next; })} /></td>
                      ...
                      <td className="px-2 py-1.5">
                        {indexed ? (
                          <div className="flex items-center justify-center gap-1">
                            <a href={`/files/${projectId}/papers/${p.paper_id}/paper.pdf`} target="_blank" rel="noopener noreferrer" className="text-red-500 hover:text-red-700 hover:bg-red-50 rounded px-1.5 py-0.5 text-xs font-medium" title="Open PDF">PDF</a>
                            {isViewingThis ? (
                              <button className="text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded px-1.5 py-0.5 text-xs font-medium" onClick={handleCloseViewer}>✕</button>
                            ) : (
                              <button className="text-indigo-600 hover:text-indigo-800 hover:bg-indigo-50 rounded px-1.5 py-0.5 text-xs font-medium" onClick={() => handleViewPaper(p.paper_id, p.title)}>👁</button>
                            )}
                            <button className="text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded px-1.5 py-0.5 text-xs font-medium" onClick={() => handleExportPaper(p.paper_id)} title="Export markdown + figures as ZIP">📥</button>
                          </div>
                        ) : (
          ...
          <div className="border border-dashed border-gray-200 rounded-lg py-6 text-center text-sm text-gray-400">No papers assigned yet. Select papers in the cart and use "Assign to" above.</div>
        )}
        {poolSelected.size > 0 && (
          <button className="mt-2 border border-red-200 bg-red-50 text-red-700 hover:bg-red-100 px-3 py-1.5 rounded-md text-xs font-medium" onClick={handleRemoveFromPool}>Remove Selected ({poolSelected.size})</button>
        )}
      </div>

      {/* Action Buttons — single row */}
      <div className="flex items-center gap-2 flex-wrap">
        <button className="bg-amber-500 hover:bg-amber-400 text-white h-9 px-4 rounded-md text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 shadow-sm" onClick={handleIndex} disabled={isBusy} title="Download PDFs and build tree indexes for pending papers">
          {isBusy && isMyOp && busy.operation === 'index' ? (<svg className="animate-spin h-3.5 w-3.5" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>) : null}
          Index {pendingCount > 0 && <span className="bg-amber-400/60 px-1.5 rounded text-xs">{pendingCount}</span>}
        </button>
        <button className="bg-indigo-600 hover:bg-indigo-500 text-white h-9 px-4 rounded-md text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 shadow-sm" onClick={handleRetrieve} disabled={isBusy} title="Extract relevant sections from indexed papers">
        ...
        <select
          className="border border-gray-300 rounded-md h-9 px-2 text-sm bg-white cursor-pointer focus:outline-none focus:ring-2 focus:ring-violet-400 max-w-[160px]"
          value={selectedTask}
          onChange={(e) => setSelectedTask(e.target.value)}
          title={selectedTaskInfo?.description || 'Select task'}
        >
        ...
        <button
          className="bg-violet-600 hover:bg-violet-500 text-white h-9 px-4 rounded-md text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 shadow-sm"
          onClick={handleResearch}
          disabled={isBusy || !selectedTask || !selectedModel || retrievals.length === 0}
          title={retrievals.length === 0 ? 'Run Retrieve first' : 'Run research task'}
        >
        ...
        <button className="ml-auto h-9 px-3 text-sm text-red-500 hover:text-red-700 hover:bg-red-50 rounded-md font-medium transition-colors" onClick={() => { if (confirm('Delete this idea and all its contents?')) onDelete(); }} title="Delete this idea">🗑 Delete</button>
      </div>
      ...
      {reports.length > 0 && (
        <div className="space-y-2 pt-2">
          <h4 className="text-sm font-semibold text-gray-700 flex items-center gap-2">
            <span>📊</span> Reports <span className="text-gray-400 font-normal">({reports.length})</span>
          </h4>
          <div className="rounded-lg border border-gray-200 overflow-hidden">
            <table className="w-full text-xs table-fixed">
              <thead>
                  ...
                  return (
                    <tr key={r.filename} className={`h-10 transition-colors ${isViewing ? 'bg-violet-50 border-l-2 border-l-violet-400' : i % 2 === 1 ? 'bg-gray-50/40' : ''} hover:bg-violet-50/40`}>
                      ...
                      <td className="px-2 py-1.5">
                        <div className="flex items-center justify-center gap-1">
                          {isViewing ? (
                            <button className="text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded px-2 py-0.5 text-xs font-medium" onClick={handleCloseViewer}>✕ Close</button>
                          ) : (
                            <button className="text-indigo-600 hover:text-indigo-800 hover:bg-indigo-50 rounded px-2 py-0.5 text-xs font-medium" onClick={() => handleView(r.filename, r.title)}>👁 View</button>
                          )}
                          <ExportButton projectId={projectId} ideaSlug={idea.idea_slug} filename={r.filename} />
                          <button className="text-red-400 hover:text-red-600 hover:bg-red-50 rounded px-2 py-0.5 text-xs font-medium" onClick={() => handleDeleteReport(r.filename, r.title)} title="Delete report">🗑</button>
                        </div>
                      </td>
                    </tr>
                  );
      ...
      {retrievals.length > 0 && (
        <div className="space-y-2">
          <h4 className="text-sm font-semibold text-gray-700 flex items-center gap-2">
            <span>📄</span> Retrieved Papers <span className="text-gray-400 font-normal">({retrievals.length})</span>
          </h4>
          <div className="rounded-lg border border-gray-200 overflow-hidden">
            <table className="w-full text-xs table-fixed">
              <thead>
                      ...
                      <td className="px-2 py-1.5">
                        <div className="flex items-center justify-center gap-1">
                          {isViewing ? (
                            ...
                            <button className="text-indigo-600 hover:text-indigo-800 hover:bg-indigo-50 rounded px-2 py-0.5 text-xs font-medium" onClick={() => handleView(r.filename, r.paper_title)}>👁 View</button>
                          )}
                          <ExportButton projectId={projectId} ideaSlug={idea.idea_slug} filename={r.filename} />
                        </div>
                      </td>
      ...
      {reports.length === 0 && retrievals.length === 0 && papers.length > 0 && (
        <div className="border border-dashed border-gray-200 rounded-lg py-6 text-center text-sm text-gray-400">
          Run <strong>Retrieve</strong> to generate paper extractions, then select a task and click <strong>▶ Run</strong> to create reports.
        </div>
      )}
      ...
      {viewContent && (
        <div className="border border-gray-200 rounded-lg max-h-[600px] overflow-auto bg-white shadow-inner">
          <div className="sticky top-0 bg-white border-b border-gray-100 px-4 py-2 flex items-center justify-between z-10">
            ...
            <button className="text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded p-1 transition-colors" onClick={handleCloseViewer} title="Close viewer">
